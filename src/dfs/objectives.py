...
def weights_for(profile: str, ctx: SeasonContext, entry_fee: float = 0.0,
    ...
    if profile == "friends_league":
        ...
        # UNITS: season_marginal_value returns $ per +1 point per week SUSTAINED over
        # the remaining schedule. A lineup's E[pts] is ONE week, so divide by weeks
        # left. Multiplying the sustained rate by a single week's score overstates the
        # objective by ~W and made a $1.51 edge look like $21.
