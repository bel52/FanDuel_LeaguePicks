...
def generate_pool(slate: PlayerSlate, spec: ContestSpec, n: int = 150,
    ...
    # Showdown only: MVP ROTATIONS. The MVP choice is the single largest decision in a
    # single-game lineup (1.5x points AND 1.5x salary), and the drop-and-resolve pivots
    # above rarely surface it because the solver re-picks the same projection-optimal
    # MVP. Rotating the MVP over an existing player set is a distinct legal lineup
    # (Candidate.key includes mvp_id) and costs no solve. Runs BEFORE the drop-pivots:
    # placed after them it never fired, because they saturate the pool first.
    if spec.slate_type == SlateType.SINGLE_GAME and candidates:
        byid = {p.fd_id: p for p in pool_players}
        for base in list(candidates):
            if len(candidates) >= n:
                break
            for pid in base.player_ids:
                if len(candidates) >= n:
                    break
                if pid == base.mvp_id or pid not in byid:
                    continue
                prem = (spec.mvp_salary_mult - 1.0)
                base_salary = sum(byid[i].salary for i in base.player_ids if i in byid)
                charged = base_salary + int(round(prem * byid[pid].salary))
                if charged > spec.salary_cap:
                    continue
                rot = Candidate(
                    player_ids=base.player_ids, salary=charged,
                    proj_sum=round(sum(byid[i].projection
                                       * (MVP_MULTIPLIER if i == pid else 1.0)
                                       for i in base.player_ids if i in byid), 2),
                    mvp_id=pid, stack_team=base.stack_team)
                if rot.key in seen:
                    continue
                seen.add(rot.key)
                candidates.append(rot)
    # Applies to BOTH slate types. This was gated to FULL, which silently capped every
    # showdown pool at n//2 (phase 1's iteration count) — half the requested candidates
    # never existed, and the showdown test only asserted the pool was non-empty.
    if candidates:
...
def _solve_showdown(players: list[SlatePlayer], spec: ContestSpec, stack: StackRule,
    ...
    for ix in teams.values():
        # FanDuel single-game requires only that BOTH teams appear. On a 6-man roster
        # that makes 5-1 legal — a heavy stack on the favorite is a standard showdown
        # shape. The old cap of 4 was correct for the 5-man roster and silently
        # illegal-ized 5-1 after the roster fix.
        prob += pulp.lpSum(x[i] for i in ix) >= 1
        prob += pulp.lpSum(x[i] for i in ix) <= ROSTER_SHOWDOWN_SIZE - 1
