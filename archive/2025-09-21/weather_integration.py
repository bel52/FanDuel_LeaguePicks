...
class WeatherDataCollector:
    ...
    async def _fetch_open_meteo(self, stadium: StadiumInfo, game_time: datetime) -> Optional[WeatherConditions]:
        ...
        try:
            ...
            # Convert game_time to timezone-naive for comparison
            game_time_naive = game_time.replace(tzinfo=None) if game_time.tzinfo else game_time
            closest_idx = min(range(len(times)), 
                            key=lambda i: abs(times[i] - game_time_naive))
