...
def format_lineup_table(lineup_players, width=100) -> str:
    ...
    for p in lineup_players:
        ...
        # OWN% from float or raw string like "20-30%-"
...
def build_text_report(result, width=100) -> str:
    ...
    if auto_locked or locked or banned or not_found:
        ...
        if auto_locked:
            cons_lines.append(f"Auto-locked: {', '.join(auto_locked)}")
        if locked:
            cons_lines.append(f"Locked: {', '.join(locked)}")
        if banned:
            cons_lines.append(f"Banned: {', '.join(banned)}")
        if not_found:
            cons_lines.append(f"Not found: {', '.join(not_found)}")
        cons_lines.append("")  # spacing
        cons_block = "\n".join(cons_lines)
    ...
    # Analysis: uppercase known headings and wrap
