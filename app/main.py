...
def health_check():
    health_status = {
        "status": "healthy",
        "components": {"api": "operational", "data": "unknown"},
    }
    try:
...
@app.get("/data/status")
def data_status():
    status = {"input_files": {}, "player_count": 0, "last_update": None}
    ...
    try:
        ...
        if df is not None:
            ...
            status["positions"] = (
                df["POS"].value_counts().to_dict() if "POS" in df.columns else {}
            )
    except Exception as e:
        status["error"] = str(e)
    return status
...
def _run_optimization(
    ...
    if players_df is None or players_df.empty:
        raise HTTPException(
            status_code=422,
            detail="No player data available. Please place CSVs under data/input/",
        )
    ...
    if auto_late_swap:
        try:
            ...
            # Convert auto-locked names to indices and merge into manual locks
            if auto_locked_names:
                auto_idx, _ = match_names_to_indices(auto_locked_names, players_df)
                lock_idx_manual.extend(auto_idx)
        except Exception as e:
            logger.warning(f"Auto-lock phase failed: {e}")
    ...
    if not lineup:
        raise HTTPException(
            status_code=422, detail="No feasible lineup found with given constraints"
        )
    ...
    result = {
        ...
        "cap_usage": {"total_salary": total_salary, "remaining": salary_cap - total_salary},
    ...
    if auto_late_swap:
        try:
            ...
            logger.warning(f"Failed to save last lineup: {e}")
...
def optimize_endpoint(
    ...
    try:
        return _run_optimization(
            salary_cap, enforce_stack, min_stack_receivers, lock, ban, auto_late_swap, game_type
        )
...
def schedule_endpoint():
    """
    Returns kickoff map and which names from the last saved lineup should be auto-locked,
    based on current time vs kickoff.
    """
    try:
        players_df = data_ingestion.load_weekly_data()
        kickoff_map = build_kickoff_map(players_df)
